...
Projection properties behaves like readonly properies where the data is fetched by a custom query. These properties are queryable like any other property (i.e. via the QueryBuilder). They serve as a means to avoid unnecessary round trips to the database
```csharp
...
To implement MagicNumber in or Dwarf class we can either chose to reference one Gem:
```csharp
[DwarfProperty]
